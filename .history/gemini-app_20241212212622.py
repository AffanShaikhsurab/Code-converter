...
def run_code(code: str, testCase: str, timeout: int = 30) -> Tuple[bool, str]:
    """
    Execute Python code with a test case safely and comprehensively.
...
def improve_recipe(code: str, error: str, testCase: str) -> Code:
    ...
    """
    print(f"imporving the code with error {error} .... ")

    # Use the existing conversion function, but provide context about the error
    llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash-002", google_api_key=GEMINI_API_KEY, temperature=0.8)
    ...
    print("code imporved successfully .. ")
