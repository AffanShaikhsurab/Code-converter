...
def get_recipe( file_path: str, context: str) -> Code:
    ...
    chat_completion = groq.chat.completions.create(
        messages=[
            {
                "role": "system",
                "content": f" Context: {context}\n. You are an Software developer engineer specialized in converting legacy (Cobol) code to new code (Python) with a single simple assert testCase to test the code . create me an python-Code for the following given cobol code the code is as follows:" + content + "convert this code into  JSON\n"
    ...
    return ConvertedFile(file_name=os.path.basename(file_path), python_code=output.python_code)
