...
async def summarize_file(file_path: str) -> FileSummary:
    ...
    if not str(file_path).endswith(('.cbl', '.cob', '.cobol')):
        return FileSummary(
            file_name=os.path.basename(file_path),
            summary="Non-COBOL file - skipped from analysis"
        )
    ...
    try:
        ...
        summary_dir = "./cobol/summary/"
        os.makedirs(summary_dir, exist_ok=True)

        summary_path = os.path.join(summary_dir, os.path.basename(file_path))
...
async def stream_process_update(message: str, progress: float = None, status: str = None) -> str:
...
async def process_files_stream(folder_path: str, files: List[UploadFile]) -> AsyncGenerator[str, None]:
    """
    ...
    try:
        ...
        for file in files:
            ...
            try:
                ...
                yield await stream_process_update(
                    f"Error uploading {file.filename}: {str(upload_error)}",
                    progress=(current_step + 0.2) / total_steps,
                    status="error"
                )
                await asyncio.sleep(0.1)  # Small delay to ensure streaming
                ...
                file.file.close()  # Ensure the file is properly closed

        current_step += 1

        # Step 1: Summarize files
        yield await stream_process_update(
            "Summarizing files...",
            progress=current_step/total_steps,
            status="summarizing"
        ...
        for idx, file in enumerate(all_files):
            try:
                summary = await summarize_file(file)  # Make sure to await the result
                if summary:  # Only add valid summaries
                    summaries.append(summary)
                yield await stream_process_update(
                    f"Summarized {os.path.basename(file)} ({idx + 1}/{len(all_files)})",
                    progress=(current_step + (idx + 1)/len(all_files))/total_steps,
                    status="summarizing"
                )
                await asyncio.sleep(0.1)  # Small delay to ensure streaming
                ...
                yield await stream_process_update(
                    f"Error processing {os.path.basename(file)}: {str(e)}",
                    progress=(current_step + (idx + 1)/len(all_files))/total_steps,
                    status="error"
                )
                await asyncio.sleep(0.1)  # Small delay to ensure streaming
        ...
        for idx, file in enumerate(sorted_files):
            ...
            context = "\n".join([summary.summary for summary in summaries 
                                   if summary.file_name in dependencies])

            # Convert file
            converted_file = convert_cobol_to_python(
            ...
            # Run AI agent
            yield await stream_process_update(
                f"Testing converted code for {file}...",
...
async def process_files(
    ...
    """Process files endpoint that returns a StreamingResponse"""
