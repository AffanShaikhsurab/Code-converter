...
async def process_files(
    ...
    async def stream_progress():
        try:
            # Step 1: Save uploaded files to the folder
            ...
            yield f"data: Uploaded files successfully.\n\n"

            # Step 2: Summarize all files
            all_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))]
            yield f"data: Using AI agent to summarize files...\n\n"

            summaries = [summarize_file(file) for file in all_files]
            yield f"data: Summarized {len(summaries)} files.\n\n"

            # Step 3: Analyze dependencies
            yield f"data: Using AI agent to analyze dependencies...\n\n"
            dependency_graph = analyze_dependencies(summaries)
            yield f"data: Dependency graph created with {len(dependency_graph.edges)} edges.\n\n"

            # Step 4: Traverse the graph and convert files
            yield f"data: Traversing dependency graph and converting files...\n\n"
            ...
            for file in sorted_files:
                ...
                yield f"data: Converting {file} with context from dependencies: {dependencies}\n\n"
                converted_file = convert_cobol_to_python(os.path.join(folder_path, file), context)

                yield f"data: Using AI agent to check the code execution...\n\n"
                ...
                yield f"data: Successfully converted {file}.\n\n"

            yield f"data: Processing complete.\n\n"
            yield f"data: {json.dumps({'status': 'success', 'converted_files': converted_files})}\n\n"
