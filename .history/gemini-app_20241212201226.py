...
class Code(BaseModel):
    """
    Pydantic model to structure the conversion output.
    
    Attributes:
        python_code (str): The converted Python code
        test_assertion (str): A test case for validating the converted code
        explanation (Optional[str]): Explanation of the conversion process
    """
    python_code: str = Field(..., description="Fully converted Python code")
    test_assertion: str = Field(..., description="Test case to validate the converted code")
    explanation: Optional[str] = Field(None, description="Conversion process explanation")
...
def convert_cobol_to_python(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not converted_code.python_code or not converted_code.test_assertion:
