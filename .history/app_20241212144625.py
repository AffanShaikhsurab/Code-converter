...
def process_files(folder_path: str):
    ...
    for file in sorted_files:
        ...
        with open(os.path.join(folder_path, file), 'r') as cobol_file:
            cobol_code = cobol_file.read()

        # Convert file using `get_recipe`
        print(f"Converting {file} with context from dependencies: {dependencies}")
        converted_code = get_recipe(cobol_code)  # Use get_recipe to generate the Python code
        converted_files.append(Code(file_name=file, python_code=converted_code.completePythonCode))
