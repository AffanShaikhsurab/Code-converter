...
def get_recipe( file_path: str, context: str) -> Code:
    ...
    try:
        ...
        if isinstance(output , ConvertedFile):
            return ConvertedFile(file_name=os.path.basename(file_path), python_code= Code.model_validate_json(chat_completion.choices[0].message.content).completePythonCode)
        else:
...
def process_files(folder_path: str):
    ...
    for converted in converted_files:
        code = aiAgent(converted.python_code , converted.testCase)
